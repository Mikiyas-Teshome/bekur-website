...
export default function EditPortfolioPage({
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const project: PortfolioProject = await response.json();
        setFormData({
          title: project.title,
          slug: project.slug,
          description: project.description,
          content: project.content,
          galleryIds: project.gallery.map((m) => m.id),
  ...
  const handleGallerySelect = (media: Media | Media[]) => {
    ...
    setSelectedGallery(mediaArray);
    setFormData((prev) => ({
      ...prev,
      galleryIds: mediaArray.map((m) => m.id),
    }));
  ...
  const handleSave = async () => {
    try {
      const dataToSave = {
        ...formData,
        shortDescription: formData.description, // Map description to shortDescription for API
        featured: formData.featured || false,
      };

      // Create a validation schema that matches the API expectations
      ...
      const validatedData = apiSchema.parse(dataToSave);
      setSaving(true);
      ...
      <div className="flex items-center justify-end">
        <div className="flex gap-2">
          <Button onClick={handleSave} disabled={saving} className=" dark:text-white cursor-pointer">
            <Eye className="h-4 w-4 mr-2 dark:text-white" />
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {selectedGallery.length > 0 ? (
                ...
                <Button
                  variant="outline"
                  className="w-full h-32 border-dashed dark:bg-background border-footer-border/50 dark:hover:bg-background/50 cursor-pointer"
                  onClick={() => setIsMediaPickerOpen(true)}
                >
      ...
      {isTagDialogOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-accent border-footer-border/50 p-6 rounded-lg w-96">
            ...
            />
            <div className="flex gap-2">
              <Button className="dark:text-white cursor-pointer" onClick={handleCreateTag}>Create</Button>
              <Button
                variant="outline"
                className="cursor-pointer dark:text-white"
